...
public class Sparta extends JLabel implements ActionListener {
	...
	public Sparta (WorldView parentview) {
		...
		//this.setIcon(new ImageIcon("Spartaview.jpg"));
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource()== NewBuilding) {//new Building
			if(ava.getSelectedItem()=="Select")
			  ...
			  try {
				  boolean flag = false;
				  ArrayList<MilitaryBuilding> n = getSpartaControl().getMilitaryBuildings();
				  ...
				  if(flag==false){
				  this.world.getParentview().getGame().getPlayer().build((String) ava.getSelectedItem(), "Sparta");
...
public void relocate() {
	if (corr.getSelectedIndex()==0 || controlarmy.getSelectedIndex()==0 || this.world.getAvail().getSelectedIndex()==0) {
		...
		try {
			...
			for(int i1 = 0 ; i1 < a1.size();i1++) {
				if(a1.get(i1).getCurrentLocation().equals("Sparta")) {
	...
	public void initiateArmy() {
		if(myunit.getSelectedIndex()==0)
			...
			ArrayList <Army> a = this.world.getParentview().getGame().getPlayer().getControlledArmies();
			for(int i = 0 ; i < a.size();i++) {
				if(a.get(i).getCurrentLocation().equals("Sparta")) {
	...
	public void UpgradeUnit() {
		if(this.unit.getSelectedIndex()==0)
			...
			try {
				this.world.getParentview().getGame().getPlayer().recruitUnit((String) this.unit.getSelectedItem(),"Sparta");
				City mycity = getSpartaControl();
				myunit.removeAllItems();
				myunit.addItem("Select Unit to UpGrade / Relocate/ initiate Army");
                for(int i = 0 ;i<mycity.getDefendingArmy().getUnits().size();i++) {
	...
	public void UpgradeSparta() {
		ArrayList<City> City = this.world.getParentview().getGame().getPlayer().getControlledCities();
		  for(int i = 0 ;i<City.size();i++) {
			  if (City.get(i).getName().equals("Sparta")) {
				City Sparta = City.get(i);
				mineEco.removeAllItems();
				mineEco.addItem(new String("Economical buildings"));
				for(int j = 0 ;j<Sparta.getEconomicalBuildings().size();j++) {
					if(Sparta.getEconomicalBuildings().get(j) instanceof Market)
					mineEco.addItem(String.valueOf("Market : Level " + Sparta.getEconomicalBuildings().get(j).getLevel()+" Up Cost=" +Sparta.getEconomicalBuildings().get(j).getUpgradeCost())) ;
					else 
						mineEco.addItem(String.valueOf("Farm : Level " + Sparta.getEconomicalBuildings().get(j).getLevel()+" Up Cost=" +Sparta.getEconomicalBuildings().get(j).getUpgradeCost()));
						
				}
				mine.removeAllItems();
				mine.addItem(new String("Military buildings"));
				for(int j = 0 ;j<Sparta.getMilitaryBuildings().size();j++) {
					if(Sparta.getMilitaryBuildings().get(j) instanceof ArcheryRange)
					mine.addItem(String.valueOf("ArcheryRange : Level " + Sparta.getMilitaryBuildings().get(j).getLevel()+" Up Cost=" +Sparta.getMilitaryBuildings().get(j).getUpgradeCost())) ;
					else if (Sparta.getMilitaryBuildings().get(j) instanceof Barracks)
						mine.addItem(String.valueOf("Barracks:Level " + Sparta.getMilitaryBuildings().get(j).getLevel() +" Up Cost=" +Sparta.getMilitaryBuildings().get(j).getUpgradeCost()));
					else if (Sparta.getMilitaryBuildings().get(j) instanceof Stable)
						mine.addItem(String.valueOf("Stable : Level " + Sparta.getMilitaryBuildings().get(j).getLevel()+" Up Cost=" +Sparta.getMilitaryBuildings().get(j).getUpgradeCost()));
