...
public class Romene extends JLabel implements ActionListener {
	...
	public Romene (WorldView parentview) {
		...
		//this.setIcon(new ImageIcon("romeview.jpg"));
		this.setBorder(BorderFactory.createLineBorder(Color.black));
		this.setOpaque(true);
		//this.parentview.add(this);
		
	   this.back = new JLabel();
	   this.setIcon(new ImageIcon("romview.jpg"));
	   this.add(back);
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource()== NewBuilding) {//new Building
			if(ava.getSelectedItem()=="Select")
			  ...
			  try {
				  boolean flag = false;
				  ArrayList<MilitaryBuilding> n = getRomeControl().getMilitaryBuildings();
				  ...
				  if(flag==false){
				  this.world.getParentview().getGame().getPlayer().build((String) ava.getSelectedItem(), "Rome");
...
public void relocate() {
	if (corr.getSelectedIndex()==0 || controlarmy.getSelectedIndex()==0 || this.world.getAvail().getSelectedIndex()==0) {
		...
		try {
			...
			for(int i1 = 0 ; i1 < a1.size();i1++) {
				if(a1.get(i1).getCurrentLocation().equals("Rome")) {
	...
	public void initiateArmy() {
		if(myunit.getSelectedIndex()==0)
			...
			ArrayList <Army> a = this.world.getParentview().getGame().getPlayer().getControlledArmies();
			for(int i = 0 ; i < a.size();i++) {
				if(a.get(i).getCurrentLocation().equals("Rome")) {
	...
	public void UpgradeUnit() {
		if(this.unit.getSelectedIndex()==0)
			...
			try {
				this.world.getParentview().getGame().getPlayer().recruitUnit((String) this.unit.getSelectedItem(),"Rome");
				City mycity = getRomeControl();
				myunit.removeAllItems();
				myunit.addItem("Select Unit to UpGrade / Relocate/ initiate Army");
                for(int i = 0 ;i<mycity.getDefendingArmy().getUnits().size();i++) {
	...
	public void UpgradeRome() {
		ArrayList<City> City = this.world.getParentview().getGame().getPlayer().getControlledCities();
		  for(int i = 0 ;i<City.size();i++) {
			  if (City.get(i).getName().equals("Rome")) {
				City Rome = City.get(i);
				mineEco.removeAllItems();
				mineEco.addItem(new String("Economical buildings"));
				for(int j = 0 ;j<Rome.getEconomicalBuildings().size();j++) {
					if(Rome.getEconomicalBuildings().get(j) instanceof Market)
					mineEco.addItem(String.valueOf("Market : Level " + Rome.getEconomicalBuildings().get(j).getLevel()+" Up Cost=" +Rome.getEconomicalBuildings().get(j).getUpgradeCost())) ;
					else 
						mineEco.addItem(String.valueOf("Farm : Level " + Rome.getEconomicalBuildings().get(j).getLevel()+" Up Cost=" +Rome.getEconomicalBuildings().get(j).getUpgradeCost()));
						
				}
				mine.removeAllItems();
				mine.addItem(new String("Military buildings"));
				for(int j = 0 ;j<Rome.getMilitaryBuildings().size();j++) {
					if(Rome.getMilitaryBuildings().get(j) instanceof ArcheryRange)
					mine.addItem(String.valueOf("ArcheryRange : Level " + Rome.getMilitaryBuildings().get(j).getLevel()+" Up Cost=" +Rome.getMilitaryBuildings().get(j).getUpgradeCost())) ;
					else if (Rome.getMilitaryBuildings().get(j) instanceof Barracks)
						mine.addItem(String.valueOf("Barracks:Level " + Rome.getMilitaryBuildings().get(j).getLevel() +" Up Cost=" +Rome.getMilitaryBuildings().get(j).getUpgradeCost()));
					else if (Rome.getMilitaryBuildings().get(j) instanceof Stable)
						mine.addItem(String.valueOf("Stable : Level " + Rome.getMilitaryBuildings().get(j).getLevel()+" Up Cost=" +Rome.getMilitaryBuildings().get(j).getUpgradeCost()));
